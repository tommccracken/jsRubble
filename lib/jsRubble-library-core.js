...
class Particle extends AbstractWorldElement {
  ...
  constructor(px, py, vx, vy, ax, ay, fx, fy, mass, radius, fixed) {
    ...
    this.lifetime = null;
    // The attractive field strength of the particle. This can be used to simulate gravitational effects.
  ...
  integrate(time_step) {
    if (!this.fixed) {
      // Verlet integration based on mutable vector operations (slight optimisation)
      ...
      // Previous Verlet integration position update based on non-mutable vector operations (without damping)
      // this.pos = this.pos.scale(2).subtract(this.pos_previous).add(this.acc.scale(time_step * time_step));
      // Previous Verlet integration position update base on non-mutable vector operations (with damping)
  ...
  // Todo: Add an adjust velocity method
...
*/
    ...
    for (let i = 0; i < this.particles.length; i++) {
      this.particles[i].calculate_velocity(this.time_step);
      if (this.simple_world_boundary_collisions) {
        if (this.particles[i].pos.y + this.particles[i].radius > this.height) {
          this.particles[i].vel.y = -this.particles[i].coefficient_of_restitution * this.particles[i].vel.y;
          this.particles[i].pos.y = this.height - this.particles[i].radius;
        } else if (this.particles[i].pos.y - this.particles[i].radius < 0) {
          this.particles[i].vel.y = -this.particles[i].coefficient_of_restitution * this.particles[i].vel.y;
          this.particles[i].pos.y = 0 + this.particles[i].radius;
        ...
        if (this.particles[i].pos.x + this.particles[i].radius > this.width) {
          this.particles[i].vel.x = -this.particles[i].coefficient_of_restitution * this.particles[i].vel.x;
          this.particles[i].pos.x = this.width - this.particles[i].radius;
        } else if (this.particles[i].pos.x - this.particles[i].radius < 0) {
          this.particles[i].vel.x = -this.particles[i].coefficient_of_restitution * this.particles[i].vel.x;
          this.particles[i].pos.x = 0 + this.particles[i].radius;
        }
        this.particles[i].pos_previous = this.particles[i].pos.subtract(this.particles[i].vel.scale(this.time_step));
      }
    }
